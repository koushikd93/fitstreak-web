...
export default function App(){
  ...
  return <div style={AS} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}><style>{`@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&display=swap');*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;font-family:'Outfit',sans-serif}html,body{background:#0A0A0F;overflow-x:hidden;min-height:100vh;min-height:100dvh}input:focus{outline:none}button:active{transform:scale(.97);transition:transform .1s}button{font-family:inherit}@keyframes xpFloat{0%{opacity:1;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-120%) scale(1.3)}}@keyframes slideIn{from{opacity:0;transform:translateX(-10px)}to{opacity:1;transform:translateX(0)}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes scanLine{0%{top:15%;opacity:0}50%{opacity:1}100%{top:85%;opacity:0}}@keyframes fadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.6}}.screen-wrap{padding-top:calc(8px + env(safe-area-inset-top))!important;padding-bottom:calc(100px + env(safe-area-inset-bottom))!important}.nav-safe{padding-bottom:calc(10px + env(safe-area-inset-bottom))!important}::-webkit-scrollbar{display:none}`}</style>
